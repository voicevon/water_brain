...
### 2.1 ESP32 连接 HC595 串行总线引脚
主控使用 3 个 GPIO 引脚控制级联的 HC595 芯片：
- **HC595_DS_PIN** (GPIO 14): 串行数据输入引脚 (SER)
- **HC595_SH_CP_PIN** (GPIO 12): 移位寄存器时钟引脚 (SRCLK)
- **HC595_ST_CP_PIN** (GPIO 13): 存储寄存器/锁存器时钟引脚 (RCLK)

*注：以上引脚可在 `config.h` 中灵活调整。*

### 2.2 HC595 输出引脚映射表 (16-bit 寄存器分配)
HC595 控制的 16 位输出（双片 8 位级联，从低位 Bit 0 到高位 Bit 15）映射分配如下：

| 寄存器位 (Bit) | 功能定义 | 输出对象 | 描述 |
| :---: | :--- | :--- | :--- |
| **Bit 0** | Relay 0 | 继电器 / 水泵 0 | 控制通道 0 对应的水泵 |
| **Bit 1** | Relay 1 | 继电器 / 水泵 1 | 控制通道 1 对应的水泵 |
| **Bit 2** | Relay 2 | 继电器 / 水泵 2 | 控制通道 2 对应的水泵 |
| **Bit 3** | LED Ch0 | 传感器通道 0 指示灯 | 当通道 0 处于有效检测或 Active 时亮起 |
| **Bit 4** | LED Ch1 | 传感器通道 1 指示灯 | 当通道 1 处于有效检测或 Active 时亮起 |
| **Bit 5** | LED Ch2 | 传感器通道 2 指示灯 | 当通道 2 处于有效检测或 Active 时亮起 |
| **Bit 6** | LED Stage 0 | 阶段 0 指示灯 | 当前激活通道处于 Stage 0 (空闲) 时亮起 |
| **Bit 7** | LED Stage 1 | 阶段 1 指示灯 | 当前激活通道处于 Stage 1 (待稳) 时亮起 |
| **Bit 8** | LED Stage 2 | 阶段 2 指示灯 | 当前激活通道处于 Stage 2 (取头样) 时亮起 |
| **Bit 9** | LED Stage 3 | 阶段 3 指示灯 | 当前激活通道处于 Stage 3 (延时 1) 时亮起 |
| **Bit 10** | LED Stage 4 | 阶段 4 指示灯 | 当前激活通道处于 Stage 4 (取中样) 时亮起 |
| **Bit 11** | LED Stage 5 | 阶段 5 指示灯 | 当前激活通道处于 Stage 5 (延时 2) 时亮起 |
| **Bit 12** | LED Stage 6 | 阶段 6 指示灯 | 当前激活通道处于 Stage 6 (取尾样) 时亮起 |
| **Bit 13** | LED Stage 7 | 阶段 7 指示灯 | 当前激活通道处于 Stage 7 (延时 3 / 等信号丢) 时亮起 |
| **Bit 14** | LED Stage 8 | 阶段 8 指示灯 | 当前激活通道处于 Stage 8 (排空) 时亮起 |
| **Bit 15** | LED Stage 9 | 阶段 9 指示灯 | 当前激活通道处于 Stage 9 (结束) 时亮起 |
